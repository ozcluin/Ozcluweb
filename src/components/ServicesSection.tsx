...
function getIcon(iconName: string, index: number) {
  ...
  if (name?.includes('user') || name?.includes('employ')) return icons.users;
  if (name?.includes('ident') || name?.includes('biometr')) return icons.identity;
  if (name?.includes('due') || name?.includes('business') || name?.includes('corp')) return icons.business;
  if (name?.includes('compl') || name?.includes('sanct') || name?.includes('shield')) return icons.compliance;
  if (name?.includes('monitor') || name?.includes('watch')) return icons.monitoring;
  if (name?.includes('custom') || name?.includes('workflow')) return icons.custom;

  // fallback by index
  const keys = Object.keys(icons) as Array<keyof typeof icons>;
  return icons[keys[index % keys.length]] || icons.custom;
}

export default function ServicesSection() {
  const [displayedServices, setDisplayedServices] = useState(fallbackServices);
        ...
        if (res.ok) {
          ...
          if (data && data.length > 0) {
            const mapped = data.map((s: any, i: number) => ({
              icon: getIcon(s.icon || s.title, i),
              title: s.title,
              description: s.description,
              audience: s.features && s.features.length > 0 ? s.features.join(' • ') : 'For onboarding & screening operations',
            }));
